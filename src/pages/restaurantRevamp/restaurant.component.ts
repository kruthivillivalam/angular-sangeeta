...
@Component({
  ...
  export class RestaurantComponent implements OnInit {
    ...
    public sortBySelection: any;
    public selectedItemArray: any = [];
    public merchantArray: any = [];
    public sortingType: any;
    public searchKey: any;
    public searchedMerchantData:any;
    ...
    constructor(
      ...
      this.zoomlevel=13;
      this.sortBySelection = 'Cost For Two';
    }
...
getAllRestaurant(filters:any,pageNumber) {
  ...
  if(filters.locations != undefined) {
    console.log("filters.locations : "+JSON.stringify(filters.locations))
    if (filters.locations.suburb != undefined) {
      finalData['suburb'] = filters.locations.suburb;
      this.suburblocation = filters.locations.suburb;
    }
    else {
      finalData['suburb'] = filters.locations;
      this.suburblocation = filters.locations;
    }
  } else {
  ...
  if(this.localStorage.get('sideFilterData') != undefined || this.localStorage.get('sideFilterData') != null) {
    ...
    finalData['serviceId']  = this.localStorage.get('tableBookingId');
  }
  else if (this.localStorage.get('foodOrderId') !== undefined && this.localStorage.get('foodOrderId') !== null) {
    finalData['serviceId']  = this.localStorage.get('foodOrderId');
  }
  else {
    finalData['serviceId']  = sideData.toString();
  }
...
if(this.cusionData != undefined) {
  console.log("this.cusionData4 : "+JSON.stringify(this.cusionData));
  if(this.cusionData.length > 0) {
...
return new Promise(resolve => {
  console.log('finalData before post'+JSON.stringify(finalData));
  ...
  .then(Searchdata => {
    ...
    if(alldata.status == 1){
      ...
      this.merchantArray = alldata.data.merchants;
...
onSubmit (data) {
  ...
  this.localStorage.set('searchByPostCode', data.postalcode);
  this.postcodeData = {
...
searchRestaurant() {
  this.searchedMerchantData = [];
  if (this.searchKey) {
    var allMerchantData = this.allSearchData.merchants;
    for (var i = 0; i < allMerchantData.length; i++) {
      var restaurantObj = allMerchantData[i];
      if (restaurantObj.merchantname.indexOf(this.searchKey) >= 0) {
        this.searchedMerchantData.push(restaurantObj)
      }
    }
    if (this.searchedMerchantData.length > 0) {
      this.allMerchantData = [];
      this.allMerchantData = this.searchedMerchantData;

      console.log('Restaurant allMerchantData : ' + JSON.stringify(this.allMerchantData));
    }
  }
  else {
    this.allMerchantData = this.allSearchData.merchants;
  }
  this.searchKey = '';
}
...
getStateSuburbByMerchant(tree) {
  return new Promise(resolve => {
    ...
    //console.log('alldata.data for state and suburb'+JSON.stringify(this.node.data.subTitle));
...
onMapReady(map) {
  ...
  var lat = 0;
  var lng = 0;
  console.log("isSearchData : " +this.isSearchData);
  console.log("isMapViewFlag : " +this.isMapViewFlag);
  ...
  console.log("JSON.stringify(this.positions) : "+JSON.stringify(this.positions));
  this.center = this.positions[0].latlong;
}
onMapClick(event) {
...
*/
this.originmarker.resTitle = '' + pos.resAddress, this.originmarker.resAddress = '', e.nguiMapComponent.openInfoWindow('iwo', e)
}
...
sortBySelectedItem($event) {
  this.sortBySelection = $event;
  let sortBySelection = $event;
  //console.log("$event : "+JSON.stringify($event));

  var allMerchants = this.merchantArray;
  this.selectedItemArray = [];
  this.allMerchantData = [];
  allMerchants.forEach(data => {
    if (sortBySelection == 'Cost For Two') {
      if (this.selectedItemArray.indexOf(data.costft) == -1) {
        this.selectedItemArray.push(data.costft);
      }
    }
    else if (sortBySelection == 'Suburb') {
      var suburb = data.address.suburb;
      if (this.selectedItemArray.indexOf(suburb) == -1) {
        console.log("suburb : "+suburb);
        this.selectedItemArray.push(suburb);
      }
    }
    else if ((sortBySelection == 'TakeAway') || (sortBySelection == 'Delivery')) {
      if (data.service.length > 0) {
        var serviceArray = data.service;
        serviceArray.forEach(service => {
          if (service.paramname == sortBySelection) {
            this.allMerchantData.push(data);
          }
        });
      }
    }
  });
  console.log("this.allMerchantData : "+JSON.stringify(this.allMerchantData));
  const sortedArray = this.selectedItemArray.sort(function (a, b) {
    return b - a;
  });
  if (sortedArray) {
    sortedArray.forEach(sortingType => {
      sortedArray.forEach(sortingType => {
        allMerchants.forEach(resData => {
          if (resData.address.suburb == sortingType) {
            this.allMerchantData.push(resData);
            //console.log("this.allMerchantData : " +JSON.stringify(this.allMerchantData));
          }
          if (sortingType === resData.costft) {
            //console.log("this.allMerchantData : " +JSON.stringify(this.allMerchantData));
            this.allMerchantData.push(resData);
          }
        });
  ...
  onSortBySelectionChanged(sortBySelection){
    this.sortBySelection = sortBySelection;
    var allMerchants = this.merchantArray;
    this.selectedItemArray = [];
    this.allMerchantData = [];
    allMerchants.forEach(data => {
